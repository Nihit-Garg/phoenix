...
export class PeerInfoExchange {
  ...
  private async receive(sourceIp: string, sourcePort: number, value: string): Promise<void> { let signed: SignedPeerInfo; try { signed = parseSignedPeerInfo(value); } catch { return; } if (!crypto_sign_verify_detached(from_base64(signed.signature), peerInfoSigningPayload(signed.peer), from_base64(signed.peer.signingPublicKey))) return; await this.directory.upsert({ ...signed.peer, ipAddress: sourceIp, port: sourcePort as 9000, updatedAt: Date.now() }); }
}
