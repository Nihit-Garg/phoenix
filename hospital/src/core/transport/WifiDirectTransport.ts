...
export class WifiDirectTransport implements PeerTransport {
  ...
  constructor(private readonly groupOwnerIntent = 0) {}
  ...
  private runDiscoveryCycle(): void { if (this.groupFormed && !this.isGroupOwner) return; try { NativeWifiDirect.startDiscovery(); void NativeWifiDirect.getPeers().then((peers) => peers.forEach((peer) => this.recordPeer(peer))).catch(() => undefined); } catch (error) { this.emit({ type: 'error', error: error instanceof Error ? error : new Error('Unable to restart nearby discovery.') }); } }
  private autoConnect(peerId: string): void { if ((this.groupFormed && !this.isGroupOwner) || this.connectingTo) return; this.connectingTo = peerId; void this.connect(peerId).catch(() => { this.connectingTo = undefined; }); this.connectionTimeout = setTimeout(() => { this.connectingTo = undefined; this.runDiscoveryCycle(); }, 12_000); }
