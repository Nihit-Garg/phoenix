...
export class PeerInfoExchange {
  constructor(private readonly transport: PeerTransport, private readonly identity: CivilianIdentity, private readonly directory = new PeerDirectory()) {}
  start(): () => void {
    return this.transport.subscribe((event) => { if (event.type === 'message') void this.receive(event.message.from.peerId, event.message.from.ipAddress ?? '0.0.0.0', event.message.from.port ?? 9000, new TextDecoder().decode(event.message.bytes)); });
  }
  async announce(peerId: string, ipAddress = '0.0.0.0'): Promise<void> {
    const peer: PeerInfo = { version: 1, peerId: this.identity.encryptionPublicKey, displayName: 'Mirage Civilian', role: 'civilian', encryptionPublicKey: this.identity.encryptionPublicKey, signingPublicKey: this.identity.signingPublicKey, ipAddress, port: 9000, updatedAt: Date.now() };
